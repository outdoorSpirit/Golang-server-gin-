...
func TestAnnotationLogin_Login(t *testing.T) {
	(&F.AnnotationFixture{}).Generate(3)
	...
	httpTests := test.HttpTests{
		{
			...
			Check:   func(t *testing.T, rec *httptest.ResponseRecorder) {
				...
				assert.EqualValues(t, "annotator-0002", claims["sub"])
...
func TestAnnotationLogin_Logout(t *testing.T) {
	...
	httpTests := test.HttpTests{
		{
			...
			Prepare: func(req *http.Request) {
				if _, e := db.Exec(`UPDATE annotator SET token_version = 'token-0002' where id = 2`); e != nil {
			...
			Check:   func(t *testing.T, rec *httptest.ResponseRecorder) {
				...
				var actual *model.Annotator
				if r, e := db.Get(model.Annotator{}, 2); e != nil {
					assert.FailNow(t, e.Error())
				} else {
					assert.NotNil(t, r)
					actual = r.(*model.Annotator)
				}
		...
		},
		{
			...
			Token:   auth.Token(1),
			Prepare: func(req *http.Request) {
				if _, e := db.Exec(`UPDATE annotator SET token_version = 'unmatch' where id = 2`); e != nil {
					assert.FailNow(t, e.Error())
				}
			},
			Check:   func(t *testing.T, rec *httptest.ResponseRecorder) {
